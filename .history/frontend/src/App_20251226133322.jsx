import { useEffect, useState } from "react";
import "./index.css";
import {
  LineChart,
  Line,
  XAxis,
  YAxis,
  Tooltip,
  ResponsiveContainer,
} from "recharts";



function App() {
  const [recommendations, setRecommendations] = useState([]);

  const [summary, setSummary] = useState(null);
  const [waste, setWaste] = useState([]);
  const [usage, setUsage] = useState([]);
  const [anomalies, setAnomalies] = useState([]);
const API_BASE = "https://ai-cloud-cost-optimizer.onrender.com";


  useEffect(() => {
fetch(`${API_BASE}/recommendations`)
  .then(res => res.json())
  .then(data => setRecommendations(data));
  fetch(`${API_BASE}/cost-summary`)
    .then((res) => res.json())
    .then((data) => setSummary(data));

  fetch(`${API_BASE}/waste`)
    .then((res) => res.json())
    .then((data) => setWaste(data));

fetch(`${API_BASE}/usage`)
        .then((res) => res.json())
    .then((data) => setUsage(data));

  fetch(`${API_BASE}/anomalies`) // ✅ ADD THIS
    .then((res) => res.json())
    .then((data) => setAnomalies(data));
}, []);

  if (!summary) {
    return <div className="p-10">Loading...</div>;
  }

  // Prepare chart data: cost per hour
  const chartData = usage.map((row) => ({
    time: row.timestamp.slice(11, 16),
    cost: Number(row.cost),
  }));

  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <h1 className="text-3xl font-bold mb-6">
        AI Cloud Cost Optimizer Dashboard
      </h1>

      {/* Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="bg-gray-800 p-6 rounded-xl shadow">
          <h2 className="text-lg text-gray-400">Total Cost Today</h2>
          <p className="text-2xl font-bold mt-2">
            ₹{summary.total_cost_today}
          </p>
        </div>

        <div className="bg-gray-800 p-6 rounded-xl shadow">
          <h2 className="text-lg text-gray-400">Money Wasted</h2>
          <p className="text-2xl font-bold mt-2 text-red-400">
            ₹{summary.money_wasted}
          </p>
        </div>

        <div className="bg-gray-800 p-6 rounded-xl shadow">
          <h2 className="text-lg text-gray-400">
            Predicted Cost Tomorrow
          </h2>
          <p className="text-2xl font-bold mt-2 text-green-400">
            ₹{summary.predicted_cost_tomorrow}
          </p>
        </div>
      </div>

      {/* Cost Trend Chart */}
      <div className="bg-gray-800 p-6 rounded-xl shadow mb-10">
        <h2 className="text-xl font-semibold mb-4">
          Cost Trend (Hourly)
        </h2>

        <div style={{ width: "100%", height: 300 }}>
          <ResponsiveContainer>
            <LineChart data={chartData}>
              <XAxis dataKey="time" />
              <YAxis />
              <Tooltip />
              <Line
                type="monotone"
                dataKey="cost"
                stroke="#22c55e"
                strokeWidth={2}
              />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>



<h2 className="text-xl font-semibold mt-8 mb-4 text-red-400">
  ⚠️ Cost Anomaly Alerts
</h2>

{anomalies.length === 0 ? (
  <p className="text-gray-400">No anomalies detected 🎉</p>
) : (
  <div className="space-y-4">
    {anomalies.map((item, index) => (
      <div
        key={index}
        className="bg-red-900/30 border border-red-500 p-5 rounded-xl shadow"
      >
        <div className="flex justify-between items-center mb-2">
          <h3 className="text-lg font-bold text-red-400">
            🚨 {item.resource}
          </h3>
          <span className="text-sm text-gray-400">
            {item.time}
          </span>
        </div>

        <p className="text-sm mb-1">
          <span className="text-gray-400">Cost:</span>{" "}
          <span className="text-red-300 font-semibold">₹{item.cost}</span>
        </p>

        <p className="text-sm mb-1">
          <span className="text-gray-400">Reason:</span>{" "}
          {item.reason}
        </p>

        <p className="text-sm mb-1">
          <span className="text-gray-400">Likely Cause:</span>{" "}
          {item.likely_cause}
        </p>

        <p className="text-sm mt-2 text-green-400 font-medium">
          ✅ Suggested Action: {item.suggested_action}
        </p>
      </div>
    ))}
  </div>
)}






      {/* Waste Table */}
      <h2 className="text-xl font-semibold mt-10 mb-4 text-green-400">
  💡 Optimization Recommendations
</h2>

<div className="space-y-4">
  {recommendations.map((item, idx) => (
    <div
      key={idx}
      className="bg-green-900/20 border border-green-500 p-5 rounded-xl"
    >
      <h3 className="font-bold text-green-400">
        {item.resource_id}
      </h3>

      <p className="text-sm">
        Recommendation: <strong>{item.recommendation}</strong>
      </p>

      <p className="text-sm">
        Reason: {item.reason}
      </p>

      <p className="text-sm text-green-300 mt-1">
        Estimated Monthly Savings: ₹{item.estimated_monthly_savings}
      </p>
    </div>
  ))}
</div>

      <div className="bg-gray-800 p-6 rounded-xl shadow">
        <h2 className="text-xl font-semibold mb-4">
          Wasted Resources
        </h2>

        {waste.length === 0 ? (
          <p className="text-gray-400">No waste detected 🎉</p>
        ) : (
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-700">
                <th className="py-2">Resource</th>
                <th className="py-2">Idle Hours</th>
                <th className="py-2">Money Wasted</th>
              </tr>
            </thead>
            <tbody>
              {waste.map((item) => (
                <tr
                  key={item.resource_id}
                  className="border-b border-gray-700"
                >
                  <td className="py-2">{item.resource_id}</td>
                  <td className="py-2">{item.idle_hours}</td>
                  <td className="py-2 text-red-400">
                    ₹{item.money_wasted}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        )}
      </div>
    </div>
  );
}

export default App;
